...
class INS():
	...
	def Alignment_KF(self, Gyro, Acce, Pos, Ang, Filter_Number):
		...
		if self.Ali_KF_Time < Filter_Number:
	...
	def Initial_Correct(self, number):
		if self.Correct_Flag == 0:
			...
			self.Correct_Sum = (self.Correct_Sum) / number
			for i in range(number):
			...
			self.Correct_Square = (self.Correct_Square) / number
	...
	def Initial_Alignment(self, Pos, Vel, Ang, flag):#, mag, pos, vel):
		...
		if flag == 1:
			...
			print 'The alignment Pos is:' , self.Pos[0:2]
			print 'The alignment Vel is:' , self.Vel[0:2]
			print 'The alignment Ang is:' , self.Ang
		else:
			Flag_Out = False
	...
	def Correct(self, gyro, acc, Error):
		...
		if gyro[2] < self.Correct_Sum[2] + 1 * self.Correct_Square[2] and gyro[2] > self.Correct_Sum[2] - 1 * self.Correct_Square[2]:
			gyro[2] = 0
	...
	def Update_and_Correct(self, Gyro, Acce, Number, Delta_T):
		...
		if self.DAngle_K == Number:
			...
			# Update position
			# type 1
			self.Pos[0] = self.Pos[0] + self.Vel[0] * Delta_T / ( ( self.Rn + self.Vel[2] ) * math.cos( self.Pos[1] ) ) * self.deg
			self.Pos[1] = self.Pos[1] + self.Vel[1] * Delta_T / ( self.Rm + self.Vel[2] ) * self.deg
